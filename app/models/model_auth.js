...
module.exports = {
    ...
    groups: async function(id_panel_users) {
        try {
            await mysql.connectAsync()
            var sql = "SELECT id_groups,name FROM tr_panel_users_groups JOIN ms_groups ON ms_groups.id = tr_panel_users_groups.id_groups WHERE id_users =" + id_panel_users + " ORDER BY id_groups";
            var [result, cache] = await mysql.queryAsync(sql)
            await mysql.endPool()
            return [result, null]
        } catch (error) {
    ...
    test_connection: async function(id_panel_users) {
        try {
            await mysql.connectAsync()
            var sql = "SELECT 1 " +
                "FROM ms_karyawan "
